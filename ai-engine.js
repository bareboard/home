...
const SYSTEM_PROMPT = `You are CRITICAL-SYNTHESIS-AI, a merciless domain-specialized intelligence for Critical Minerals, Aggregates, Oil and EV battery metals.
...
2) THEN: 4-6 original full articles in markdown. Each must have: ### Headline, Byline "By Critical Synthesis AI", 650-950 word body dense with chemistry + economics + rates, and a **Key Takeaway** box.
...
async function initAI(progressCallback) {
  ...
  try {
    ...
    engineReady = true;
    console.log("%c[CRITICAL-SYNTHESIS-AI] engine ready", "color:#FF4200;font-weight:bold");
    return true;
  } catch (err) {
    console.error("[CRITICAL-SYNTHESIS-AI] init failed:", err);
...
async function runMercilessSynthesis(rawData) {
  ...
  console.log("%c[CRITICAL-SYNTHESIS-AI] synthesis complete (10 iterations)", "color:#FF4200;font-weight:bold");
  return finalOutput;
}

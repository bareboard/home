...
(() => {
  ...
  const writeQuote = (container, item) => {
    ...
    if (value) {
      const key = `card:${item.symbol}:${Array.from(document.querySelectorAll('[data-testid="card-container"]')).indexOf(container)}`;
      if (window.umsAnimateVerifiedValue) window.umsAnimateVerifiedValue(value, key, item.value);
  ...
  const updateTableRows = (scope, items) => {
    ...
    scope.querySelectorAll('[data-testid="data-table-v2"] tr').forEach((row) => {
      ...
      if (cells.length >= 3) {
        const key = `table:${item.symbol}:${row.rowIndex}:${scope.dataset?.testid || 'document'}`;
        if (window.umsAnimateVerifiedValue) window.umsAnimateVerifiedValue(cells[2], key, item.value);
        else cells[2].textContent = item.value;
      }

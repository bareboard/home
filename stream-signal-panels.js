...
(() => {
  ...
  const load = async () => {
    try {
      ...
      const contextSlot = document.querySelector('main [data-ums-panel="context"]') || document.querySelector('main #sda-MID-CENTER-global-1') || document.querySelector('main .ntv-insert .native-ad');
      const moversSlot = document.querySelector('main [data-ums-panel="movers"]') || document.querySelector('main #sda-NEWS-STREAM-stream-1');
      const slots = Array.from(document.querySelectorAll('main .native-ad.yf-4rju3x')).filter(slot => !slot.dataset.umsPanel || slot.dataset.umsPanel === 'signal' || slot.dataset.umsPanel === 'pulse');
